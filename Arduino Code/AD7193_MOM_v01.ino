/*
    ...
    May 7 2024: this is the current script to use on KI this summer - uploaded to all R4s
    ---  I left the debug in because it works and is fast enough (~50-60hz) using 80 samples averaging on the amplifier

    May 10, 2024
    ---  Removed the code to adjust RTC. Setting RTC in a separate script applied odnly to the SD shield. Prevents a reset to date of compile of this script
...
void loop() {
  ...
  unsigned long strain;
    ...
    myUnixTime = Get_TimeStamp(); // NOT SURE WHY THIS ISN'T UPDATING SECONDS EACH TIME
    // myUnixTime = tCounter;
  ...
  if (dataFile) {
    ...
    for (int i = 0; i < 300; i++) {
      ...
      strain = Get_Data(); // CHECK TO SEE IF THIS CAN BE FASTER BY DIRECT CALL TO THE AD7193 INSTEAD OF GET_DATA FUNCTION
    ...
    // show user we are collecting data BY SHOWING IT ON THE LCD
    // Write to LCD
    lcd.setCursor(6, 0);
